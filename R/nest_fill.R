...
#'
#' @details
#'  `nest_fill()` is a wrapper for \code{[tidyr::fill()]} and maintains the functionality
...
#' @param ... \code{<[`tidy-select`][tidyr_tidy_select]>} Columns to fill.
...
nest_fill <- function(
  ...
  dplyr::mutate(
    ...
    "{{.nest_data}}" := purrr::map(
      {{ .nest_data }},
      ~ tidyr::fill(.x, !!!dots, .direction = .direction)
    )
  )
}
